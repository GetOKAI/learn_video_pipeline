...
# Try to import the new google.genai client - fallback if not available
try:
    from google import genai as google_genai
    from google.genai import types
    VEO_CLIENT_AVAILABLE = True
except ImportError:
    print("⚠️ google.genai not available - will use fallback video generation")
    VEO_CLIENT_AVAILABLE = False
...
class VideoGenerator:
    """Generates videos from enhanced prompts using Google Veo 3.1"""
    
    def __init__(self):
        # Use consistent Gemini authentication
        self.gemini_api_key = os.getenv('GEMINI_API_KEY')
        self.project_id = os.getenv('GOOGLE_CLOUD_PROJECT')  # For Vertex AI
        self.location = os.getenv('GOOGLE_CLOUD_LOCATION', 'us-central1')  # Default location
        ...
        if not self.gemini_api_key:
            ...
            try:
                ...
                # Initialize Veo client for video generation
                if VEO_CLIENT_AVAILABLE:
                    try:
                        # Try to initialize the new google.genai client
                        self.veo_client = google_genai.Client(api_key=self.gemini_api_key)
                        print("✅ Veo 3.1 client configured")
                    except Exception as e:
                        print(f"⚠️ Could not initialize Veo client: {e}")
                        self.veo_client = None
                else:
                    print("⚠️ google.genai not available - using fallback video generation")
                    self.veo_client = None
        ...
        # Check for available TTS (keep for audio if needed)
    ...
    def _check_tts_availability(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                print("✅ Text-to-Speech available (macOS 'say' command)")
                return True
            
            # Check for other TTS options
            # You can add checks for other TTS systems here
            print("⚠️ No Text-to-Speech system found")
            return False
        except Exception:
            return False
    
    def _check_ffmpeg_availability(self) -> bool:
        """FFmpeg is no longer used - Veo generates videos directly"""
        return False
    
    def _extract_dialogue_text(self, script_content: str) -> str:
        """Extract clean dialogue text from script for TTS"""
        ...
        for line in lines:
            ...
            if (not line or 
                ...
                line.startswith('**Dialogue') or
                line.startswith('**(') or  # Skip timing markers like **(0-10 seconds)**
                line.startswith('* ') or  # Skip bullet points like "* **Mike (50s):**"
                line.startswith('(') or   # Skip stage directions
                ':' not in line):
                continue
            
            # Extract dialogue from character lines
            dialogue = ""
            if line.startswith('**') and ':**' in line:
                # Format: **MIKE:** dialogue
        ...
        if dialogue:
            # Remove stage directions in parentheses from the beginning
            if dialogue.startswith('(') and ')' in dialogue:
                # Remove everything from start until the first )
    ...
    def _generate_audio_from_script(self, script_content: str, audio_path: str) -> bool:
        ...
        try:
            dialogue_text = self._extract_dialogue_text(script_content)
            
            if not dialogue_text.strip():
                print("⚠️ No dialogue text found in script")
                return False
            
            print(f"🎤 Generating audio from script dialogue...")
            print(f"   Text preview: {dialogue_text[:100]}...")
            
            # Use macOS 'say' command to generate audio
            # You can customize voice and speed here
            cmd = [
                'say', 
    ...
    def _generate_video_with_veo(self, prompt: str, title: str) -> Optional[bytes]:
        """Generate video using Veo 3.1 API"""
        if not self.veo_client or not VEO_CLIENT_AVAILABLE:
            print("⚠️ Veo client not available")
            return None
            
        try:
            print("🎬 Generating video with Veo 3.1...")
            
            # Create video generation operation with audio
            try:
                operation = self.veo_client.models.generate_videos(
                    ...
                    config=types.GenerateVideosConfig(
                        ...
                        duration_seconds=8,
                        resolution="1080p",
                        generate_audio=True,  # Try to generate audio with Veo
                    ),
                )
                print("🎵 Audio generation enabled in Veo request")
            except Exception as e:
                error_msg = str(e)
                if "generate_audio parameter is not supported" in error_msg:
                    print("⚠️ Veo audio generation not supported, trying without audio...")
                    operation = self.veo_client.models.generate_videos(
                        model="veo-3.1-generate-preview",
                        prompt=prompt,
                        config=types.GenerateVideosConfig(
                ...
                elif any(x in error_msg for x in ["enhancePrompt", "enhance_prompt", "person_generation", "personGeneration"]):
                    print(f"⚠️ Unsupported parameter, trying minimal config...")
                    operation = self.veo_client.models.generate_videos(
                        model="veo-3.1-generate-preview",
                        prompt=prompt,
                        config=types.GenerateVideosConfig(
            ...
            if operation.response:
                ...
                print("❌ No video response received")
                return None
                
        except Exception as e:
            print(f"❌ Veo video generation error: {str(e)}")
            return None
    ...
    def generate_video(self, enhanced_prompt: str, title: str = "", model: str = "veo-3", script_content: str = "") -> Dict:
        """
        Generate actual video using Veo 3.1 from enhanced prompts
        
        Args:
            enhanced_prompt (str): The enhanced prompt text
            title (str): Video title
            model (str): Model preference (ignored - always uses Veo 3.1)
            script_content (str): Generated script content for reference
        ...
        try:
            ...
            if self.model is None or self.veo_client is None:
                # Create mock video when API key is missing
                self._create_mock_video_file(video_filepath, title, enhanced_prompt)
                return {
                    "title": title,
                    "enhanced_prompt": enhanced_prompt[:200] + "..." if len(enhanced_prompt) > 200 else enhanced_prompt,
                    "video_file": video_filename,
                    "file_path": video_filepath,
                    "file_size_mb": round(os.path.getsize(video_filepath) / (1024 * 1024), 2),
                    "generation_timestamp": datetime.now().isoformat(),
                    "model_used": f"mock-{model}",
                    "status": "success",
                    "note": "Mock video file created - set GEMINI_API_KEY for Veo videos"
                }

            # Enhance prompt for video generation using Gemini
            print("🎨 Enhancing video prompt with Gemini...")
            video_prompt_enhancement = f"""
Create a compelling video prompt for Veo 3.1 based on this content:

Title: {title}
Original prompt: {enhanced_prompt}

Transform this into a clear, visual prompt that will work well for AI video generation. Focus on:
1. Specific visual elements and scenes
2. Clear action descriptions
3. Professional, educational tone for blue-collar workers  
4. Concise but descriptive language
5. 8-second duration considerations

Make it engaging and authentic for construction/trades workers.
"""
            ...
            print(f"📝 Enhanced prompt: {enhanced_video_prompt[:150]}...")
            
            # Generate video using Veo 3.1
            video_bytes = self._generate_video_with_veo(enhanced_video_prompt, title)
            
            # Also generate separate audio file if script is available
            audio_filepath = None
            if script_content and self.tts_available:
                audio_filename = f"{safe_title}_{timestamp}.aiff"
                audio_filepath = os.path.join(self.output_dir, audio_filename)
                print("🎤 Generating separate audio file from script...")
                audio_success = self._generate_audio_from_script(script_content, audio_filepath)
            ...
            if video_bytes:
                ...
                print(f"✅ Video saved: {video_filepath} ({file_size / (1024*1024):.2f} MB)")
                ...
                result = {
                    ...
                    "aspect_ratio": "9:16",
                    "resolution": "1080p",
                    "has_separate_audio": audio_filepath is not None,
                    "audio_file": os.path.basename(audio_filepath) if audio_filepath else None,
                    "status": "success",
                    "note": "Video generated with Veo 3.1" + (" + separate audio" if audio_filepath else "")
                }
                
                return result
            else:
                # Fall back to enhanced production guide
                print("🎬 Creating enhanced video production guide...")
                self._create_enhanced_video_file(video_filepath, title, enhanced_prompt, enhanced_video_prompt)
                
                file_size = os.path.getsize(video_filepath) if os.path.exists(video_filepath) else 0
                ...
                result = {
                    ...
                    "status": "success",
                    "note": "Enhanced production guide created (Veo unavailable)"
                }
                
                print(f"✅ Enhanced production guide created: {video_filepath}")
                return result
...
- Format: MP4, 1080x1920 (9:16 vertical), 30fps
- Duration: 30-60 seconds  
...
class CompletePipeline:
    ...
    def process_single_prompt(self, enhanced_prompt: str, title: str = "", 
        ...
        # Get script content for audio generation
        script_content = script_result.get("actor_script", "") if script_result.get("status") in ["success", "success_mock"] else ""
